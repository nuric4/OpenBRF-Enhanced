...
bool MainWindow::exportAnimationManyFiles(){
  assert(selector->currentTabName()==ANIMATION);

  std::vector<int> sel = selector->allSelected();
  if (sel.empty()) {
    statusBar()->showMessage(tr("No animations selected."), 2000);
    return false;
  }
  ...
  if (!s) {
    QMessageBox::information(this,
      tr("Open Brf"),
      tr("Cannot export animations without a proper skeleton!\n")
    );
    return false;
  }
  ...
  QString dir = QFileDialog::getExistingDirectory(
    this,
    tr("Select folder for SMD animation export"),
    path,
  ...
  settings->setValue("LastExpImpPath", dir);
  ...
  for (size_t k = 0; k < sel.size(); ++k) {
    int i = sel[k];
    if (i < 0 || i >= (int)brfdata.animation.size()) {
      ++failed;
      continue;
    }

    BrfAnimation const &a(brfdata.animation[i]);
    QString fn = QDir(dir).filePath(QString::fromLatin1(a.name) + ".SMD");

    // Overwrite an existing file with the same animation name.
  ...
  QString summary = tr("Exported %1 animation(s) to:\n%2")
    .arg(exported)
  ...
  QMessageBox::information(this, tr("Open Brf"), summary);
  return failed == 0;
}
