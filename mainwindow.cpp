...
void MainWindow::breakAniMany(std::vector<int> indexes, bool useIni){
	...
	if (!useIni) {
		...
		for (size_t k=0; k<indexes.size(); ++k) {
			int which = indexes[k];
			if (which < 0 || which >= (int)brfdata.animation.size()) continue;
			BrfAnimation ani = brfdata.animation[which];
			int res = ani.Break(brfdata.animation);
			if (res>0) total += res;
		}
		...
		statusBar()->showMessage(tr("Split %1 selected animations into %2 chunks.").arg((int)indexes.size()).arg(total), 5000);
		return;
	}
	...
	for (size_t k=0; k<indexes.size(); ++k) {
		...
		if (res>0) {
			...
			total += res;
			processed++;
			lastTxt = QString::fromStdWString(std::wstring(newTxt));
		}
	}
	...
	if (processed>0) {
		...
		setModified();
		statusBar()->showMessage(
			tr("Split %1 selected animations into %2 chunks -- new animation.txt file saved in \"%3\"!")
			.arg(processed).arg(total).arg(lastTxt), 8000);
	} else {
		statusBar()->showMessage(tr("Nothing to split (or none could be split)."), 5000);
	}
}
