...
/*
 * Modified in August 2026 for the OpenBRF Enhanced fork.
 * Changes include list search, bulk animation export, and SMD scale
 * compatibility/fix (new exports use 1:1 scale while legacy imports remain
 * supported).
 */
...
Selector::Selector(QWidget *parent)
  : QTabWidget(parent), findBox(NULL), findNextRow(0)
{
	...
	breakAniWithIniManyAct = new QAction(tr("Split selected via action.txt..."), this);
	breakAniWithIniManyAct->setStatusTip(tr("Split all selected animations using the same action.txt file."));
	...
	exportAnimationManyAct = new QAction(tr("Export selected animations..."), this);
	exportAnimationManyAct->setStatusTip(tr("Export all selected animations as separate SMD files."));
	...
	connect(breakAniWithIniManyAct, SIGNAL(triggered()),this,SLOT(onBreakAniWithIniMany()));
	...
	connect(exportAnimationManyAct, SIGNAL(triggered()),parent,SLOT(exportAnimationManyFiles()));
	...
	// Quick search for the current tab (especially useful for animations).
	// Ctrl+F focuses the box; Enter jumps to the next matching entry.
	findBox = new QLineEdit(this);
	findBox->setPlaceholderText(tr("Find..."));
	findBox->setToolTip(tr("Find by name in the current list. F3 focuses this box; Enter finds the next match."));
	findBox->setMaximumWidth(240);
	setCornerWidget(findBox, Qt::TopRightCorner);
	connect(findBox, SIGNAL(textChanged(QString)), this, SLOT(findTextChanged(QString)));
	connect(findBox, SIGNAL(returnPressed()), this, SLOT(findNext()));
	QShortcut *findShortcut = new QShortcut(QKeySequence("F3"), this);
	connect(findShortcut, SIGNAL(activated()), this, SLOT(focusFindBox()));
...
void Selector::updateContextMenu(){
	...
	if (!nosel) {
		...
		if (t==ANIMATION) {
			...
			if (mulsel) {
				contextMenu->addAction(exportAnimationManyAct);
				contextMenu->addAction(breakAniWithIniManyAct);
				contextMenu->addAction(aniMergeAct);
			}
...
void Selector::onBreakAniWithIniMany(){
	QModelIndexList selected = tab[ANIMATION]->selectionModel()->selectedIndexes();
	std::vector<int> indexes;
	for (QModelIndex idx : selected) indexes.push_back(idx.row());
	if (!indexes.empty()) emit breakAniMany(indexes, true);
}
...
void Selector::findTextChanged(const QString &text){
	// Do not search while the user is typing. Searching on every
	// keystroke steals focus from the QLineEdit and causes the
	// next character to be handled by the animation list.
	findText = text;
	findNextRow = 0;
}

void Selector::findNext(){
	if (findText.isEmpty()) return;

	int kind = currentTabName();
	if (kind < 0 || kind >= N_TOKEN) return;

	MyTableModel *model = tableModel[kind];
	QListView *view = tab[kind];
	if (!model || !view || model->size() == 0) return;

	QString needle = findText.trimmed();
	if (needle.isEmpty()) return;

	const int count = model->size();
	for (int offset = 0; offset < count; ++offset){
		int row = (findNextRow + offset) % count;
		if (model->vec[row].contains(needle, Qt::CaseInsensitive)){
			selectOne(kind, row);
			findNextRow = (row + 1) % count;
			return;
		}
	}
}

void Selector::focusFindBox(){
	if (!findBox) return;
	findBox->setFocus();
	findBox->selectAll();
}
